...
class Sensor {
  ...
  /// Return the last x-coordinate in the [y]-slice of the coverage of this
  /// sensor.
  ///
  /// This method only returns a valid result if [y] is within the coverage
  /// area.
  int lastXInY(int y) {
    return position.x + radius - (position.y - y).abs();
  }
...
List<Position> extractBeacons(List<String> input) {
  return input
      ...
      .map((numberList) => Position(numberList[0], numberList[1]))
      .toList();
}
...
int part2(List<String> input) {
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      // 'Swing' from current x to the last x-coordinate of the sensors,
      // skipping evaluation of many x-coordinates in the process.
      while (sensors.any((sensor) => sensor.coversPosition(Position(x, y)))) {
        for (Sensor sensor in sensors) {
          if (sensor.coversPosition(Position(x, y))) {
            newX = sensor.lastXInY(y) + 1;
          }
        }
        x = newX;
      }

      // If the last swing resulted in an x within our evaluation bounds, and
      // the position is not a known beacon, it must be the beacon we are
      // looking for.
      if (x < size && !beacons.any((beacon) => beacon == Position(x, y))) {
        return 4000000 * x + y;
      }
    }
  }
