...
/// Abstraction of items on the system.
///
/// Can be either a [SystemFile] or a [SystemDirectory].
abstract class SystemItem {
  ...
  /// Create a [SystemFile] or a [SystemDirectory], based on the [output].
  factory SystemItem.fromOutput({
    ...
    if (parts[0] == 'dir') {
      return SystemDirectory(parent: parent, name: parts[1]);
    } else {
...
///
/// Its [files] are initially yet to be discovered through calling [ls] with
...
class SystemDirectory extends SystemItem {
  ...
  // Files are initially undiscovered.
  List<SystemItem> files = const [];

  @override
  int get size {
    return files
        .map((file) => file.size)
        .fold(0, (totalSize, fileSize) => totalSize + fileSize);
  }

  /// Returns the [SystemDirectory] that is navigated to.
  ///
  /// Accepts:
  /// - '/': navigates to root directory
  /// - '..': navigates to parent directory
  /// - '$name': navigates to directory with name $name
  SystemDirectory cd(String location) {
    if (location == '/') {
      return parent?.cd(location) ?? this;
    }
    if (location == '..') {
      return parent ?? this;
    }
    return files.firstWhere(
            (file) => file.name == location && file is SystemDirectory)
        as SystemDirectory;
  }

  /// Adds files to this [SystemDirectory] based of [lsOutput].
  void ls(List<String> lsOutput) {
    this.files = lsOutput
  ...
  @override
  String toPrettyString() {
    String contentsString = files
        .map((file) =>
            '\n  ${file.toPrettyString().split('\n').map((fileContent) => '  $fileContent').join('\n')}')
        .join();
    return '${this}$contentsString';
  }
}

/// Construct a directory structure from [input].
///
/// Output is a [SystemDirectory] called '/' containing all files described by
/// the [input].
SystemDirectory constructDirectoryStructure(List<String> input) {
  /// Add `cd /` command to ensure `ls` output is flushed and we end up at the
  /// root directory.
  input.add('\$ cd /');

  SystemDirectory currentDirectory = SystemDirectory(name: '/');

  // Buffer output from `ls` command.
  ...
  input.forEach((line) {
    ...
    bool inputFound = lineWords[0] == '\$';

    // Output has ended.
    if (inputFound && !readingInput) {
      // Flush buffered `ls` output.
      currentDirectory.ls(lsOutput);
      lsOutput.clear();
    }

    readingInput = inputFound;

    if (readingInput) {
      if (lineWords[1] == 'cd') {
        currentDirectory = currentDirectory.cd(lineWords[2]);
      }
    } else {
      // Buffer `ls` output line.
      lsOutput.add(line);
    }
  });
...
/// Flatten directory structure by returning a [List] of all [SystemDirectory]s
/// in [root], excluding [root].
List<SystemDirectory> flattenDirectories(SystemDirectory root) {
  return root.files.whereType<SystemDirectory>().toList() +
      root.files
          .whereType<SystemDirectory>()
          .map((directory) => flattenDirectories(directory))
          .fold([], (directoryList, directory) => directoryList + directory);
}
...
int part1(List<String> input) {
  final SystemDirectory root = constructDirectoryStructure(input);
...
int part2(List<String> input) {
  ...
  final int requiredDiskSpace = 30000000;
  final int usedDiskSpace = root.size;
  final int missingDiskSpace =
      requiredDiskSpace - (totalDiskSpace - usedDiskSpace);

  final List<SystemDirectory> sortedQualifyingDirectories =
      flattenDirectories(root)
          .where((element) => element.size >= missingDiskSpace)
          .toList()
        ..sort((a, b) => a.size - b.size);

  return sortedQualifyingDirectories.first.size;
}
