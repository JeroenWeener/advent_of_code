...
void main() {
  List<String> input = File('2022/dart/03/input.txt').readAsLinesSync();
...
/// Helper method to convert [Runes] value to [1-52], where a=1, b=2, z=26,
/// A=27, Z=52.
int runeToPriority(int rune) {
  return rune > 'Z'.runes.first
      ? rune - 'a'.runes.first + 1
      : rune - 'A'.runes.first + 1 + 26;
}
...
int part1(List<String> input) {
  Iterable<int> runes = input.map(
    (String line) {
      String compartmentA = line.substring(0, line.length ~/ 2);
      String compartmentB = line.substring(line.length ~/ 2);
      Set<int> runesCompartmentA = compartmentA.runes.toSet();
      Set<int> runesCompartmentB = compartmentB.runes.toSet();
      return runesCompartmentA
          .firstWhere((rune) => runesCompartmentB.contains(rune));
    },
  );

  // Convert runes to priorities and sum values.
  return runes
      .map((rune) => runeToPriority(rune))
      .reduce((total, rune) => total + rune);
}
...
int part2(List<String> input) {
  // Preprocess input to groups of 3.
  List<List<String>> lines = input.fold([[]], (previousValue, element) {
    if (previousValue.last.length == 3) {
      previousValue.add([]);
    }
  ...
  Iterable<int> badges = lines.map((e) {
    Set<int> runesRucksackA = e[0].runes.toSet();
    Set<int> runesRucksackB = e[1].runes.toSet();
    Set<int> runesRucksackC = e[2].runes.toSet();

    return runesRucksackA.firstWhere((rune) =>
        runesRucksackB.contains(rune) && runesRucksackC.contains(rune));
  });

  // Convert runes to priorities and sum values.
  return badges
      .map((badge) => runeToPriority(badge))
      .reduce((total, rune) => total + rune);
}
