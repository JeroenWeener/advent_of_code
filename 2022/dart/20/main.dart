...
void main() {
  ...
  BigInt answer2 = part2(input);
  print(answer2);
}

class RingElement {
  RingElement(this.value);

  final int value;
  late RingElement predecessor;
  late RingElement successor;

  /// Return the [RingElement] [i] places from this.
  ///
  /// If [i] is larger than the size of the ring, this iterative algorithm will
  /// pass this specific element at one point. This method is 'exclusive' in the
  /// sense that it will not count this element when progressing.
  ///
  /// For example:
  ///
  ///   Ring: 6,1,2,3,4
  ///   Starting from '6' in the ring, element i has value j:
  ///     i: j
  ///     1: 1
  ///     2: 2
  ///     3: 3
  ...
  ///     5: 1
  ///     6: 2
  RingElement getRelativeElementExclusive(int i) {
    return _getRelativeElementExclusive(this, i);
  }

  RingElement _getRelativeElementExclusive(RingElement starter, int i) {
    if (i == 0) {
      return this;
    }

    return i > 0
        ? successor._getRelativeElementExclusive(
            starter, i - (successor == starter ? 0 : 1))
        : predecessor._getRelativeElementExclusive(
            starter, i + (predecessor == starter ? 0 : 1));
  }

  /// Return the [RingElement] [i] places from this.
  ///
  /// If [i] is larger than the size of the ring, this iterative algorithm will
  /// pass this specific element at one point. This method is 'inclusive' in the
  /// sense that it will count this element when progressing.
  ///
  /// For example:
  ///
  ///   Ring: 6,1,2,3,4
  ...
  ///     5: 6
  ///     6: 1
  RingElement getRelativeElementInclusive(int i) {
    if (i == 0) {
      return this;
    }

    return i > 0
        ? successor.getRelativeElementInclusive(--i)
        : predecessor.getRelativeElementInclusive(++i);
  }

  /// Shift this element in its ring by [value] items.
  ///
  /// This procedure will not count the element itself when shifting, should
  /// [value] exceed the size of the ring. See [getRelativeElementExclusive].
  void move() {
    if (value == 0) {
      return;
    }
    ...
    if (value > 0) {
      RingElement newPredecessor = getRelativeElementExclusive(value);
      if (newPredecessor == this) {
        newPredecessor = successor;
      }
      RingElement newSuccessor = newPredecessor.getRelativeElementExclusive(1);
    ...
    if (value < 0) {
      RingElement newSuccessor = getRelativeElementExclusive(value);
      if (newSuccessor == this) {
...
int part1(List<String> input) {
  /// Store [RingElement]s in a list. This list will not change order throughout
  /// solving the puzzle.
  List<RingElement> elements =
      input.map((value) => RingElement(int.parse(value))).toList();

  /// Populate every [RingElement] with its predecessor and successor.
  ...
  /// Mix the file by moving every element in order.
  elements.forEach((element) => element.move());

  /// Find the zero element.
  RingElement zero = elements.firstWhere((element) => element.value == 0);

  /// Calculate the sum of the grove coordinates. When finding the grove
  /// coordinates, we count elements inclusive.
  return [1000, 2000, 3000]
      .map((groveCoordinate) =>
          zero.getRelativeElementInclusive(groveCoordinate).value)
      .reduce((a, b) => a + b);
}
...
BigInt part2(List<String> input) {
  int decryptionKey = 811589153;

  /// Create the ring again. Instead of applying the decryption key directly, we
  /// use mathematics to shrink the numbers dramatically.
  ///
  /// A [RingElement] with a value that is equal to the size of the ring minus 1
  /// will not move. We use this fact by shrinking first the decryption key and
  /// later the resulting value in the element by applying the modulo operator.
  List<RingElement> elements = input
      .map((value) => RingElement(
          ((decryptionKey % (input.length - 1)) * int.parse(value)) %
              (input.length - 1)))
      .toList();

  /// Populate every [RingElement] with its predecessor and successor again.
  for (int i = 0; i < elements.length; i++) {
    RingElement element = elements[i];
    RingElement formerElement = elements[(i - 1) % elements.length];
    RingElement laterElement = elements[(i + 1) % elements.length];
  ...
  /// This time, we apply the mix 10 times.
  for (int i = 0; i < 10; i++) {
    elements.forEach((element) => element.move());
  }
  ...
  /// Find the 3 elements corresponding to the grove coordinates. Then, find
  /// their index in [elements] and calculate the actual values by fetching the
  /// original numbers and applying the decryption key.
  return [1000, 2000, 3000]
      .map((groveCoordinate) =>
          BigInt.parse(input[elements
              .indexOf(zero.getRelativeElementInclusive(groveCoordinate))]) *
          BigInt.from(decryptionKey))
      .reduce((a, b) => a + b);
}
