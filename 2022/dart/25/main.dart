...
void main() {
  List<String> input = File('2022/dart/25/input.txt').readAsLinesSync();
...
List<List<int>> parseInput(List<String> input) {
  return input.map((line) => numberToSegments(line)).toList();
}

List<int> fullAdder(int a, int b, int carry) {
  int remainder = ((a + b + carry + 2) % 5) - 2;
  int sum = a + b + carry;
  int newCarry = sum > 2
      ? 1
      : sum < -2
          ? -1
          : 0;
  return [newCarry, remainder];
}

List<int> addSegments(List<int> as, List<int> bs) {
  int length = as.length > bs.length ? as.length : bs.length;
  List<int> result = [];
  int carry = 0;

  for (int segmentIndex = 0; segmentIndex < length; segmentIndex++) {
    int a = as.length > segmentIndex ? as[as.length - 1 - segmentIndex] : 0;
    int b = bs.length > segmentIndex ? bs[bs.length - 1 - segmentIndex] : 0;
    List<int> r = fullAdder(a, b, carry);
    carry = r[0];
    result.add(r[1]);
  }
  if (carry != 0) {
...
/// Converts the number to segments (= -> -2, - -> -1, 0 -> 0, 1 -> 1, 2 -> 2).
List<int> numberToSegments(String number) {
  return number.codeUnits.map((codeUnit) {
    if (codeUnit == '='.codeUnits.first) {
      return -2;
    } else if (codeUnit == '-'.codeUnits.first) {
      return -1;
    } else {
      return int.parse(String.fromCharCode(codeUnit));
    }
  }).toList();
}

/// Converts the number segments (a list containing ints ranging from -2 to 2,
/// inclusive) to the string representation.
String segmentsToNumber(List<int> segments) {
...
/// Sums multiple segment lists to a single segment list.
List<int> sum(List<List<int>> segmentLists) {
  List<int> sum = [];
  for (List<int> number in segmentLists) {
    sum = addSegments(sum, number);
  }
  return sum;
}
