...
void main() {
  List<String> input = File('2022/dart/10/input.txt').readAsLinesSync();
...
///
/// Stores the [cycles] the operation takes to complete.
abstract class Instruction {
  int get cycles;

  /// Parse [line] to an [Instruction].
  factory Instruction.fromLine(String line) {
    List<String> parts = line.split(' ');
    if (parts[0] == 'addx') {
      return AddXInstruction(int.parse(parts[1]));
    } else {
...
/// A noop instruction.
///
/// Takes 1 cpu cycle and does not do anything.
...
/// An instruction to add a [value] to a register.
///
/// Takes 2 cpu cycles.
class AddXInstruction implements Instruction {
  const AddXInstruction(this.value);
...
/// A processing unit able to determine either the signal strength or render an
/// image.
///
/// Calling [tick], [signalStrength] or [renderImage] will update the internal
/// state of the CPU. It is therefore unable to evaluate [instructions]
/// multiple times. If you want to calculate both the [signalStrength] and
/// [renderImage] from the same list of instructions, instantiate two CPUs.
class CPU {
  CPU(this.instructions) : remainingInstructionCycles = instructions[0].cycles;

  /// The value in [X].
  int registerX = 1;

  /// The next cycle the CPU will process.
  int instructionPointer = 0;

  /// The remaining cycles for the current instruction to complete.
  int remainingInstructionCycles;

  /// The remaining instructions.
  ///
  /// These are instructions that have not been completed. If an instruction is
  /// in progress, it has to be the first instruction in [instructions].
  ...
  /// Start the next instruction or continue the previous one if it has not
  /// completed.
  ///
  /// Updates both [remainingInstructionCycles] and [instructionPointer].
  void tick() {
    // Check if the current instruction has completed.
    if (remainingInstructionCycles == 0) {
      // Carry out [AddXInstruction].
      if (instructions.first is AddXInstruction) {
        registerX += (instructions.first as AddXInstruction).value;
      }

      // Remove completed instruction from remaining instructions.
      instructions.removeAt(0);

      // Set remaining cycles according to the next instruction.
      if (instructions.isNotEmpty) {
        remainingInstructionCycles = instructions.first.cycles;
      }
    }
  ...
  /// Determine the signal strength based on the [instructions].
  ///
  /// The signal strength can be calculated by:
  ///
  /// 20 * x_20 + 60 * x_60 + 100 * x_100 + 140 * x_140 + 180 * x_180 + 220 *
  /// x_220, where x_y is the value of [registerX] during the yth cycle.
  int signalStrength() {
    List.filled(20, null).forEach((_) => tick());
    return 20 * registerX +
        [60, 100, 140, 180, 220].map(
          (cycleCount) {
            List.filled(40, null).forEach((_) => tick());
            return cycleCount * registerX;
          },
        ).reduce((total, product) => total + product);
  }

  /// Render an image with a width of 40 pixels from [instructions].
  String renderImage() {
    int pixelLocation = 0;
    ...
    while (!isFinished()) {
      tick();
      image += pixelLocation >= registerX - 1 && pixelLocation <= registerX + 1
          ? '#'
          : '.';
      pixelLocation = (pixelLocation + 1) % 40;
      if (pixelLocation == 0) {
        image += '\n';
      }
    }
...
///   addx -1, so the value of register X is the starting value, 1, plus all of
///   the other addx values up to that point: 1 + 15 - 11 + 6 - 3 + 5 - 1 - 8 +
///   13 + 4 = 21.)
...
int part1(List<String> input) {
  List<Instruction> instructions =
      input.map((line) => Instruction.fromLine(line)).toList();
